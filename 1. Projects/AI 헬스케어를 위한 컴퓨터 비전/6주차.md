...
2. $f(x_i; \mathbf{p})$**:
    ...
    - xix_ixi​ 입력값에 대해 모델 fff가 계산한 출력값.
    - p\mathbf{p}p는 모델의 매개변수(예: 직선의 기울기와 절편).
    - 예를 들어, 선형 모델의 경우 f(xi;p)=mxi+bf(x_i; \mathbf{p}) = m x_i + bf(xi​;p)=mxi​+b가 될 수 있습니다.
3. $y_iyi​**:
    
    - iii번째 데이터 포인트의 실제 관측값.
