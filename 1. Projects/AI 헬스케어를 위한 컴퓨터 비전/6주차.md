...
1. **호모그래피 정의**:
    ...
    - 두 점 집합 간의 관계는 p′=Hpp' = Hp로 표현됩니다.
    - 여기서 pp와 p′p'는 각각 원본 점과 대응 점을 호모그래피 행렬 HH로 변환한 결과입니다.
2. **호모그래피 행렬**:
    
    - HH는 3×33 \times 3 크기의 행렬이며, h00,h01,…,h22h_{00}, h_{01}, \dots, h_{22}로 구성된 9개의 요소를 포함합니다.
    - 이 행렬은 스케일 팩터를 제외하고 정의됩니다.
3. **제약 조건**:
    
    - Frobenius 노름(∣∣H∣∣F||H||_F)을 1로 설정하여 해를 안정화하고 스케일을 고정합니다.
    - ∣∣h∣∣2=1||h||^2 = 1 조건이 추가됩니다. 여기서 hh는 행렬 HH의 요소를 1차원 벡터로 펼친 것입니다.
4. **최적화 문제**:
    
    - min⁡∣∣Ah−b∣∣2\min ||Ah - b||^2를 해결해야 합니다.
    - 여기서 AA와 bb는 주어진 점 집합에서 계산된 행렬입니다.
    - 이 문제는 제약 조건 ∣∣h∣∣2=1||h||^2 = 1 하에서 해결됩니다.
