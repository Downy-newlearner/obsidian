...
**3줄 요약**  
FIR 시스템에서는 현재 출력이 과거 입력의 유한 합으로 표현된다. 입력 $x[n]$을 단위 임펄스 $\delta[n]$으로 대체하여, FIR 시스템의 임펄스 응답 $h[n]$을 계산한다. 결과적으로 유한 길이의 응답이 나타난다.
...
**1줄 요약**  
FIR 시스템의 임펄스 응답은 유한한 길이를 가지며 과거 입력의 유한 합으로 구성된다.
